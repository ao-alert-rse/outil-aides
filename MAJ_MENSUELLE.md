...
1. ~~Régions hors IdF~~ — **considéré clos le 2026-07-02** : toutes les régions ont désormais une aide spécifique en plus du portail générique, sauf la Corse (écartée intentionnellement, non pertinente pour la clientèle Nam & Kouji). Pays de la Loire (2), Occitanie (2), Normandie (1) et PACA (2) ajoutées le 2026-07-02. **Point de vigilance résolu** : Aide au conseil RH (PACA) — confirmé 404 réel (page supprimée du site, pas un faux positif anti-bot) via navigateur, aucun remplaçant fiable trouvé → retirée. CEDRE Ambition (PACA) reste en base, l'appel semblait clos au moment de la vérification — à recontrôler avant de le proposer à un client.
...
3. ~~Alerte plafond de minimis~~ — **fait le 2026-07-02** : champ `minimis` (true/false/null) ajouté aux 124 aides par classification heuristique (OPCO/mutualisé, CEE, prêts/garanties/fonds propres → false ; conseil/diagnostic subventionné État/ADEME/Bpifrance/Région → true ; FSE+/FEDER/agences de l'eau/cas mixtes → null "à vérifier"). Alerte affichée dans la barre de cumul (%, dépassement, note si aide au régime incertain sélectionnée). **Classification indicative, pas un avis juridique** — à faire valider par un cas réel avant usage client si un dossier approche du plafond.
...
- ~~Champ `date_fin` sous-exploité~~ — **complété le 2026-07-02** : au passage, corrigé une date erronée sur AAP Première Usine (2027-04-06 ne correspondait à aucune relève réelle → 2026-09-08, prochaine relève après celle du 7 avril déjà passée) et ajouté PDC SSSMS OPCO Santé (2026-08-31, déduit du texte de la fiche). Recherche exhaustive faite sur les descriptions déjà en base (mots-clés date/relève/clôture) — aucune autre échéance calendaire ferme trouvée parmi les 124 aides ; le reste des dispositifs OPCO sont à enveloppe budgétaire continue, pas à date fixe.
...
6. ~~Audit anti-"vitrine"~~ — **fait et étendu le 2026-07-02**, suite à un signalement sur Fonds Chaleur ADEME (retiré : ni taux ni plafond, fiche officielle = page de contact/accompagnement au cas par cas, pas une aide actionnable). Deux passes : (1) repérage des `taux`/`plafond` en "N/A" ou "Variable" puis vérification source par source, (2) repérage élargi (mots-clés "sur dossier"/"suspendu"/"contribution conventionnelle"/etc.) + vérification systématique des liens pointant vers une page racine/générique plutôt qu'une fiche précise. **8 aides retirées au total** : CEC Parcours, Aides Agence de l'eau, PDC OPCO2i, Bilan de compétences AKTO Propreté, Fonds SPI2 (aucun terme concret ou nécessitant un contact/connexion pour la moindre info) ; Bonus écologique véhicule (confirmé arrêté par décret depuis le 01/07/2025) ; PDC AFDAS 50+ et PDC OCAPIAT 50+ (fin de la mutualisation en 2019 pour ce segment — plus vraiment une aide externe, la fiche source le dit elle-même). **5 aides corrigées** avec les vrais chiffres/liens trouvés sur les fiches officielles : Accélérateur Décarbonation Bpifrance (56 %/39 000 €, lien qui pointait vers une page de catégorie), Compétitivité énergétique Nouvelle-Aquitaine (taux 30-65 % selon profil), Garantie Développement Vert Bpifrance (quotité 80 %, lien corrigé), Cap Décarbonation MTE (lien vers la fiche précise, effectif corrigé à <250 salariés, réservé aux entreprises d'artisanat), ACT Pas-à-Pas Pacte Entreprises (taux 60-80 %, plafond 18 000 €). Base passée de 124 à **115 aides**. **Limite assumée** : pas d'audit ligne par ligne exhaustif des 115 restantes — seules celles avec un signal clair (N/A, "Variable", lien racine, formulations d'incertitude) ont été vérifiées ; les dispositifs OPCO à barème conventionnel (coût-contrat, alternance) n'ont pas été requestionnés, leur variabilité étant structurelle et non un signe de vitrine. Note technique : une commande de nettoyage a normalisé les fins de ligne du fichier en CRLF (cohérent avec le reste du dépôt) — gros diff Git sur ce commit mais aucun impact fonctionnel (syntaxe revalidée).
7. ~~Vérification HTTP de tous les liens~~ — **fait le 2026-07-02** : script Node vérifiant le code HTTP des 79 liens uniques de la base (hors blocages anti-bot connus bpifrance.fr/ocapiat.fr, testés ici en fetch direct donc non affectés). **2 liens cassés trouvés** : PDF OCAPIAT (404, fichier renommé sans le suffixe "-OCAPIAT" → corrigé, 3 aides concernées) ; Aide au conseil RH PACA (404 confirmé via navigateur réel, page supprimée du site sans remplaçant identifiable → aide retirée). Base à **114 aides**.
8. ~~Checkup général de l'outil~~ — **fait le 2026-07-02** : test fonctionnel complet en preview (recherche SIRET réel, détection OPCO, filtres, sélection/cumul, export PDF, réinitialisation), relecture du moteur de matching (`matcherAides`, `detecterOPCO`), test responsive mobile. **4 bugs/imprécisions trouvés et corrigés** : (1) bandeau d'avertissement figé sur "juin 2026" malgré la vérification massive de juillet ; (2) bug d'affichage mobile — le bouton Export PDF chevauchait le texte de la barre de cumul ; (3) l'alerte de plafond de minimis (ajoutée cette session) n'avait aucun style d'impression, texte quasi invisible sur un export PDF ; (4) 7 aides à plafond réellement variable au cas par cas (AAP, appels à projets régionaux) affichaient à tort le badge "Sur consultation" au lieu de "Montant individualisé" faute du flag `plafond_variable`. **Limites connues, non corrigées (contraintes structurelles de l'API gratuite, pas des bugs)** : l'effectif entreprise est dérivé de la tranche INSEE (borne haute de la fourchette), donc un peu approximatif pour les seuils d'effectif non alignés sur les bornes réelles ; le code NAF utilisé est celui du siège social, qui peut être un code "holding/immobilier" non représentatif de l'activité réelle pour les grands groupes multi-établissements (ex. Decathlon → 70.10Z alors que l'activité réelle est le commerce de sport). Aucune erreur console JS détectée sur l'ensemble des parcours testés.

## Infra V1 — passage à une app partagée (2026-07-03)

L'outil n'est plus un simple fichier local : il est désormais **déployé et partagé entre conseillers**.

- **Site en ligne** : `https://ao-alert-rse.github.io/outil-aides/` (dépôt GitHub public `ao-alert-rse/outil-aides`, hébergement GitHub Pages gratuit). Domaine personnalisé pas encore décidé (à ajouter plus tard : fichier `CNAME` + config DNS).
- **Base d'aides migrée vers Supabase** (projet `vxveaslsypiakmshklne`, table `aides`, 114 lignes), chargée dynamiquement au démarrage au lieu d'être codée en dur dans le HTML. Mettre à jour une aide ne demande plus de modifier le code ni de redéployer — ça se fait directement dans le Table Editor Supabase ou via l'éditeur SQL.
- **Authentification par invitation** : chaque conseiller doit être invité individuellement (Dashboard Supabase → Authentication → Users → Invite user). Pas d'auto-inscription sur le site public, volontairement.
- **Sécurité RLS** : la table `aides` n'est lisible qu'aux utilisateurs connectés (policy `aides_select_authenticated`), pour ne pas exposer gratuitement la base compilée à n'importe quel visiteur du site public.
- **Point de vigilance corrigé pendant le déploiement** : la première version de `schema.sql` préparée pour le dépôt public contenait par erreur les `INSERT` avec les 114 aides en clair — ça aurait annulé la protection RLS (n'importe qui aurait pu lire la base sans se connecter en consultant le fichier sur GitHub). Repéré et bloqué automatiquement avant tout push. Le fichier public ne contient maintenant que la structure de la table ; les données complètes (`seed-data-aides.sql`) restent uniquement dans ce dossier privé, jamais dans le dépôt public.
- **Le fichier `outil-aides-rse.html` de ce dossier reste la copie de référence/travail** pour les futures modifications de logique (matching, UI) — mais **la donnée vivante (les 114 aides) est désormais dans Supabase, pas dans ce fichier**. Après toute modification de la base d'aides dans `outil-aides-rse.html`, il faudra la répercuter dans Supabase (script de migration réutilisable : `generate_schema.js`, technique utilisée cette session).

**Reste à faire (V2/V3, prochaine session)** :
- Dossiers clients + historique des aides perçues, pour un vrai suivi du plafond de minimis dans le temps (pas juste sur la sélection du jour)
