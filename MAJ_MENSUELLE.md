...
6. ~~Audit anti-"vitrine"~~ — **fait le 2026-07-02**, suite à un signalement sur Fonds Chaleur ADEME (retiré : ni taux ni plafond, fiche officielle = page de contact/accompagnement au cas par cas, pas une aide actionnable). Étendu à toute la base (repérage des `taux`/`plafond` en "N/A" ou "Variable" puis vérification source par source) : **5 aides retirées** (CEC Parcours, Aides Agence de l'eau, PDC OPCO2i, Bilan de compétences AKTO Propreté, Fonds SPI2 — toutes sans terme concret ou nécessitant un contact/connexion pour la moindre info) ; **3 aides corrigées** avec les vrais chiffres trouvés sur les fiches officielles (Accélérateur Décarbonation Bpifrance : 56 %/39 000 €, lien correctif car il pointait vers une page de catégorie et non la fiche ; Compétitivité énergétique Nouvelle-Aquitaine : taux 30-65 % selon profil ; Garantie Développement Vert Bpifrance : quotité 80 %, lien corrigé). Base passée de 124 à 118 aides. **Pas fait un audit ligne par ligne des 118 restantes** — seules celles avec un signal clair (N/A ou "Variable" dans taux/plafond) ont été vérifiées ; les dispositifs OPCO à barème conventionnel (coût-contrat, alternance) n'ont pas été requestionnés, leur variabilité étant structurelle et non un signe de vitrine.
